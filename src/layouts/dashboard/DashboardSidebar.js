  ...
  useEffect(() => {
    ...
    if (tokenData) {
      ...
      if (!user) {
        ...
        setRole('Réception PCA');
      } else if (user && user.role_id === 3) {
        setRole('Réception Direction Générale');
      } else if (user && user.role_id === 4) {
        setRole('Directeur Général');
      } else if (user && user.role_id === 5) {
        setRole('Directeur Général Adjoint');
      } else if (user && user.role_id === 6) {
        setRole('Président Conseil Administration');
      } else if (user && user.role_id === 7) {
        setRole('Coordination Direction Générale');
      } else if (user && user.role_id === 8) {
        setRole('Assistant PCA');
      } else if (user && user.role_id === 9) {
        setRole('Assistant DG');
      } else if (user && user.role_id === 10) {
        setRole('Assistant DGA');
      } else if (user && user.role_id === 11) {
        setRole('Protocole PCA');
      } else if (user && user.role_id === 12) {
        setRole('Protocole DG');
      } else if (user && user.role_id === 13) {
        setRole('Protocole DGA');
      }
    } else {
      navigate('/', { replace: true });
    }
