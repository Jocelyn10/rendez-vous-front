...
import {
  RendezvousdgaMoreMenu,
  RendezvousdgaListToolbar
} from '../../components/_dashboard/rendezvousdga';
...
export default function Rendezvousdga() {
  ...
  const [rendezvousdgaTab, setRendezvousdgaTab] = useState([]);
  ...
  const [ajouteparInput, setAjoutepar] = useState('');
  ...
  useEffect(() => {
    axios(`${process.env.REACT_APP_BASE_URL}/rendezvousdga/`, {
      ...
      .then((value) => {
        setRendezvousdgaTab(value.data);
      })
      .catch(() => {});
  }, []);
  ...
  useEffect(() => {
    axios(`${process.env.REACT_APP_BASE_URL}/confirmationdga/`, {
      headers: {
        Authorization: `Bearer ${process.env.REACT_APP_TOKEN}`
      }
    })
  ...
  const addRendezvousdga = (
    ...
    if (nameInput !== '' && nameInput !== null && motifInput !== '' && motifInput !== null)
      axios
        .post(
          `${process.env.REACT_APP_BASE_URL}/rendezvousdga/`,
          {
            name: nameInput,
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = setRendezvousdgaTab.map((n) => n.name);
  ...
  const emptyRows =
    page > 0 ? Math.max(0, (1 + page) * rowsPerPage - setRendezvousdgaTab.length) : 0;

  const filteredRendezvousdga = applySortFilter(
    rendezvousdgaTab,
    getComparator(order, orderBy),
    filterName
  );

  const isUserNotFound = filteredRendezvousdga.length === 0;
      ...
      <Container>
        <Stack direction="row" alignItems="center" justifyContent="space-between" mb={5}>
          <Typography variant="h4" gutterBottom>
            Rendez-vous Directeur Général Adjoint
          </Typography>
          <Button
            variant="contained"
        ...
        <Card>
            ...
            <TableContainer sx={{ minWidth: 800 }}>
              <Table>
                <UserListHead
                  ...
                  rowCount={setRendezvousdgaTab.length}
                  numSelected={selected.length}
                  onRequestSort={handleRequestSort}
                  onSelectAllClick={handleSelectAllClick}
                />
                <TableBody>
                  {filteredRendezvousdga
          ...
          <TablePagination
            ...
            component="div"
            count={setRendezvousdgaTab.length}
